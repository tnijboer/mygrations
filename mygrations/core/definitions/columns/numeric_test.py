...
class NumericTest(unittest.TestCase):
    ...
    def test_default_errors(self):
        self.assertEqual([f"Column 'test' of type 'DECIMAL' cannot have a string value as a default"],
                          Numeric('test', 'DECIMAL', default='').schema_errors)
        self.assertEqual([f"Column 'test' of type 'INT' must have an integer value as a default"],
                          Numeric('test', 'INT', default=5.0).schema_errors)
        self.assertEqual([f"Column 'test' of type 'BIT' must have a default of 1 or 0"],
    ...
    def test_length_errors(self):
        self.assertEqual([f"Column 'test' of type 'FLOAT' cannot have a length"],
                          Numeric('test', 'FLOAT', length=5).schema_errors)
        self.assertEqual([f"Column 'test' of type 'INT' must have an integer value as its length"],
                          Numeric('test', 'INT', length='5,2').schema_errors)

    def test_character_set_errors(self):
        self.assertEqual([f"Column 'test' of type 'INT' cannot have a character set"],
                          Numeric('test', 'INT', character_set='UTF-8').schema_errors)
        self.assertEqual([f"Column 'test' of type 'INT' cannot have a collate"],
                          Numeric('test', 'INT', collate='UTF-8').schema_errors)

    def test_auto_increment_errors(self):
        self.assertEqual([f"Column 'test' of type 'FLOAT' cannot be an AUTO_INCREMENT"],
